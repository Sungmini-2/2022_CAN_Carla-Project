...
#        self.can_bus.send(message) # like this make send_steer, send_gear with honda.dbc
#        print(message)


class CAN(object):
    def __init__(self):
        self.db = cantools.database.load_file('/home/jo/tmp/PythonAPI/examples/honda.dbc')
    ...
    def send_speed(self, speed):
        ID_1=random.choice(range(1,99))
        data = self.speed_message.encode({'WHEEL_SPEED_FL' : speed, 'WHEEL_SPEED_FR': speed, 'WHEEL_SPEED_RL': speed, 'WHEEL_SPEED_RR': speed})
        speed_ID=(self.speed_message.frame_id)
        result= ID_1 + speed_ID
        #message = can.Message(arbitration_id = self.speed_message.frame_id, data=random.sample(range(1,100),8))
        message = can.Message(arbitration_id = result, data=data) 
        
        self.can_bus.send(message) # like this make send_steer, send_gear with honda.dbc
        print(message)

    def send_steer(self, steer):
        ID_2=random.choice(range(1,88))
        data = self.steer_message.encode({'STEER_ANGLE' : steer * 10})
        steer_ID=(self.steer_message.frame_id)
        result= ID_2 + steer_ID
        message = can.Message(arbitration_id=result, data=data)
        #message = can.Message(arbitration_id=self.steer_message.frame_id, data=random.sample(range(1,100),8))
        self.can_bus.send(message)
        print(message)

    def send_gear(self, gear):
         ID_3=random.choice(range(1,44))
         gear_ID=(self.gear_message.frame_id)
         result= ID_3 + gear_ID
         if(gear == 0):
...
class HUD(object):
    ...
    def tick(self, world, clock):
        ...
        if isinstance(c, carla.VehicleControl):
            self.can.send_steer(c.steer)
            self.can.send_gear(c.gear)

            self._info_text += [
                ('Throttle:', c.throttle, 0.0, 1.0),
                ('Steer:', c.steer, -1.0, 100),
