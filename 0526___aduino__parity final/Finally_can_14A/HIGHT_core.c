#include "HIGHT_core.h"

const BYTE Delta[128] PROGMEM= {
	};

const BYTE HIGHT_F0[256] PROGMEM= {
	};

const BYTE HIGHT_F1[256] PROGMEM= {
	};

void    HIGHT_Key(BYTE    *pbUserKey, BYTE    *pdwRoundKey){
	int     i, j;

	for( i=0; i<4; i++) {
		pdwRoundKey[i  ] = pbUserKey[i+12];
		pdwRoundKey[i+4] = pbUserKey[i   ];
	}

	for( i=0; i<8; i++) {
		for( j=0; j<8; j++)
			pdwRoundKey[8+16*i+j  ] = (BYTE) (pbUserKey[(j-i)&7    ] + pgm_read_byte(&Delta[16*i+j  ]));
		for( j=0; j<8; j++)
			pdwRoundKey[8+16*i+j+8] = (BYTE) (pbUserKey[((j-i)&7)+8] + pgm_read_byte(&Delta[16*i+j+8]));
	}
}

void    HIGHT_Enc(BYTE    *pdwRoundKey,BYTE    *pbData){
	BYTE   XX[8];
	BYTE	cnt=2;
	BYTE	i;

	XX[1] = pbData[1];
	XX[3] = pbData[3];
	XX[5] = pbData[5];
	XX[7] = pbData[7];

	XX[0] = (pbData[0] + pdwRoundKey[0]);
	XX[2] = (pbData[2] ^ pdwRoundKey[1]);
	XX[4] = (pbData[4] + pdwRoundKey[2]);
	XX[6] = (pbData[6] ^ pdwRoundKey[3]);

	#define HIGHT_Enc_MACRO(k, i0,i1,i2,i3,i4,i5,i6,i7) {                         \
		XX[i0] = (XX[i0] ^ ( pgm_read_byte(&HIGHT_F0[XX[i1]]) + pdwRoundKey[4*k+3]));    \
		XX[i2] = (XX[i2] + ( pgm_read_byte(&HIGHT_F1[XX[i3]]) ^ pdwRoundKey[4*k+2]));    \
		XX[i4] = (XX[i4] ^ ( pgm_read_byte(&HIGHT_F0[XX[i5]]) + pdwRoundKey[4*k+1]));    \
		XX[i6] = (XX[i6] + ( pgm_read_byte(&HIGHT_F1[XX[i7]]) ^ pdwRoundKey[4*k+0]));    \
	}

	for (i=0;i<4;i++)
	{		
		HIGHT_Enc_MACRO( cnt,  7,6,5,4,3,2,1,0); cnt++;
		HIGHT_Enc_MACRO( cnt,  6,5,4,3,2,1,0,7); cnt++;
		HIGHT_Enc_MACRO( cnt,  5,4,3,2,1,0,7,6); cnt++;
		HIGHT_Enc_MACRO( cnt,  4,3,2,1,0,7,6,5); cnt++;
		HIGHT_Enc_MACRO( cnt,  3,2,1,0,7,6,5,4); cnt++;
		HIGHT_Enc_MACRO( cnt,  2,1,0,7,6,5,4,3); cnt++;
		HIGHT_Enc_MACRO( cnt,  1,0,7,6,5,4,3,2); cnt++;
		HIGHT_Enc_MACRO( cnt,  0,7,6,5,4,3,2,1); cnt++;
	}
	
	pbData[1] = (BYTE) XX[2];
	pbData[3] = (BYTE) XX[4];
	pbData[5] = (BYTE) XX[6];
	pbData[7] = (BYTE) XX[0];

	pbData[0] = (BYTE) (XX[1] + pdwRoundKey[4]);
	pbData[2] = (BYTE) (XX[3] ^ pdwRoundKey[5]);
	pbData[4] = (BYTE) (XX[5] + pdwRoundKey[6]);
	pbData[6] = (BYTE) (XX[7] ^ pdwRoundKey[7]);
}

void    HIGHT_Dec(BYTE    *pdwRoundKey,BYTE    *pbData){
	DWORD   XX[8];
	BYTE	cnt=33;
	BYTE	i;

	XX[2] = (BYTE) pbData[1];
	XX[4] = (BYTE) pbData[3];
	XX[6] = (BYTE) pbData[5];
	XX[0] = (BYTE) pbData[7];

	XX[1] = (BYTE) (pbData[0] - pdwRoundKey[4]);
	XX[3] = (BYTE) (pbData[2] ^ pdwRoundKey[5]);
	XX[5] = (BYTE) (pbData[4] - pdwRoundKey[6]);
	XX[7] = (BYTE) (pbData[6] ^ pdwRoundKey[7]);

	#define HIGHT_Dec_MACRO(k, i0,i1,i2,i3,i4,i5,i6,i7) {                         \
		XX[i1] = (XX[i1] - (pgm_read_byte(&HIGHT_F1[XX[i2]]) ^ pdwRoundKey[4*k+2])) & 0xFF;    \
		XX[i3] = (XX[i3] ^ (pgm_read_byte(&HIGHT_F0[XX[i4]]) + pdwRoundKey[4*k+1])) & 0xFF;    \
		XX[i5] = (XX[i5] - (pgm_read_byte(&HIGHT_F1[XX[i6]]) ^ pdwRoundKey[4*k+0])) & 0xFF;    \
		XX[i7] = (XX[i7] ^ (pgm_read_byte(&HIGHT_F0[XX[i0]]) + pdwRoundKey[4*k+3])) & 0xFF;    \
	}

	for (i=0;i<4;i++)
	{
		HIGHT_Dec_MACRO(cnt,  7,6,5,4,3,2,1,0);cnt--;
		HIGHT_Dec_MACRO(cnt,  0,7,6,5,4,3,2,1);cnt--;
		HIGHT_Dec_MACRO(cnt,  1,0,7,6,5,4,3,2);cnt--;
		HIGHT_Dec_MACRO(cnt,  2,1,0,7,6,5,4,3);cnt--;
		HIGHT_Dec_MACRO(cnt,  3,2,1,0,7,6,5,4);cnt--;
		HIGHT_Dec_MACRO(cnt,  4,3,2,1,0,7,6,5);cnt--;
		HIGHT_Dec_MACRO(cnt,  5,4,3,2,1,0,7,6);cnt--;
		HIGHT_Dec_MACRO(cnt,  6,5,4,3,2,1,0,7);cnt--;
	}

	pbData[1] = (BYTE) (XX[1]);
	pbData[3] = (BYTE) (XX[3]);
	pbData[5] = (BYTE) (XX[5]);
	pbData[7] = (BYTE) (XX[7]);

	pbData[0] = (BYTE) (XX[0] - pdwRoundKey[0]);
	pbData[2] = (BYTE) (XX[2] ^ pdwRoundKey[1]);
	pbData[4] = (BYTE) (XX[4] - pdwRoundKey[2]);
	pbData[6] = (BYTE) (XX[6] ^ pdwRoundKey[3]);
}
